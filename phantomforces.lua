...
local Window = Library.CreateLib("Luaware - Phantom Forces", "GrapeTheme")

local shais = sett_2


-- I didn't think this ui lib through
local SilentTab = Window:NewTab("Combat")
local SilentSection = SilentTab:NewSection("Silent aim")

SilentSection:NewToggle("Enable silent aim", "Enables the silent aim", function(state)
    shais.silentaim = state
end)

SilentSection:NewSlider("Headshot hance", "This is the chance of hitting a headshot", 100, 0, function(v) -- 500 (MaxValue) | 0 (MinValue)
    shais.headshotchance = v
end)

SilentSection:NewSlider("FOV Radius", "Changes the size of the FOV circle", 1000, 5, function(v) -- 500 (MaxValue) | 0 (MinValue)
    shais.fov = v
end)

SilentSection:NewToggle("Enable FOV Circle", "Enables the silent aim's circle", function(state)
    shais.fovcircle = state
end)

SilentSection:NewSlider("FOV Sides", "Changes the sides of the FOV circle", 64, 3, function(v) -- 500 (MaxValue) | 0 (MinValue)
    shais.fovsides = v
end)

local PlrTab = Window:NewTab("Player")
local MoveSection = PlrTab:NewSection("Movement")

MoveSection:NewToggle("Disable fall damage", "Disables fall damage", function(state)
    shais.nofalldamage = state
end)

MoveSection:NewToggle("Enable speed hacks", "Enables the sonic hacks", function(state)
    shais.setwalkspeed = state
    if (not state) then
...
MoveSection:NewSlider("Speed amount", "Changes the speed of you", 250, 0, function(v) -- 500 (MaxValue) | 0 (MinValue)
    shais.walkspeed = v
    storage.char.setbasewalkspeed(char, v)
end)

MoveSection:NewToggle("Enable jump hacks", "Enables the jump hacks", function(state)
    shais.setjumppower = state
end)

MoveSection:NewSlider("Jump amount", "Changes the jump of you", 250, 0, function(state) -- 500 (MaxValue) | 0 (MinValue)
    shais.jumppower = state
end)

local VisTab = Window:NewTab("Visuals")
local MiscSection = VisTab:NewSection("Misc")

MiscSection:NewLabel("Gun visuals will update when you respawn")

MiscSection:NewToggle("Colour effect", "Changes the colour", function(state)
    shais.colorEffect = state
    storage.colorEffect.Enabled = state
end)

MiscSection:NewToggle("Gun chams", "Changes the colour of your gun", function(state)
    shais.gunVisuals = new
end)

MiscSection:NewToggle("Dead effect", "Changes the colour of dead", function(state)
    shais.deadVisuals = state
end)
